...
void test_cifar_multi(char *filename, char *weightfile)
{
    ...
    for(i = 0; i < test.X.rows; ++i){
        ...
        int classify = max_index(test.y.vals[i], 10);
        if(index == classify) avg_acc += 1;
        free_image(im);
        printf("%4d: %.2f%%\n", i, 100.*avg_acc/(i+1));
    }
}
...
void extract_cifar()
{
    char *labels[] = {(char *)"airplane",(char *)"automobile",(char *)"bird",(char *)"cat",(char *)"deer",(char *)"dog",(char *)"frog",(char *)"horse",(char *)"ship",(char *)"truck"};
    int i;
    data train = load_all_cifar10();
    data test = load_cifar10_data((char *)"data/cifar/cifar-10-batches-bin/test_batch.bin");
    for(i = 0; i < train.X.rows; ++i){
        image im = float_to_image(32, 32, 3, train.X.vals[i]);
        int classify = max_index(train.y.vals[i], 10);
        char buff[256];
        sprintf(buff, "data/cifar/train/%d_%s",i,labels[classify]);
    ...
    for(i = 0; i < test.X.rows; ++i){
        ...
        int classify = max_index(test.y.vals[i], 10);
        char buff[256];
        sprintf(buff, "data/cifar/test/%d_%s",i,labels[classify]);
        save_image_options(im, buff, PNG, 0);
    }
}
