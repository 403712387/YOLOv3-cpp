...
int bbox_comparator(const void *a, const void *b)
{
    ...
    free_image(im2);
    free(X);
    if (predictions[classify*2] > predictions[classify*2+1]){
        return 1;
    }
    return -1;
}

void bbox_update(sortable_bbox *a, sortable_bbox *b, int classify, int result)
{
    int k = 32;
    float EA = 1./(1+pow(10, (b->elos[classify] - a->elos[classify])/400.));
    float EB = 1./(1+pow(10, (a->elos[classify] - b->elos[classify])/400.));
    float SA = result ? 1 : 0;
    float SB = result ? 0 : 1;
    a->elos[classify] += k*(SA - EA);
    b->elos[classify] += k*(SB - EB);
}

void bbox_fight(network net, sortable_bbox *a, sortable_bbox *b, int classes, int classify)
{
...
void BattleRoyaleWithCheese(char *filename, char *weightfile)
{
    ...
    for (classify = 0; classify < classes; ++classify){

        N = total;
        current_class = classify;
        ...
        for(round = 1; round <= 100; ++round){
            ...
            for(i = 0; i < N/2; ++i){
                bbox_fight(net, boxes+i*2, boxes+i*2+1, classes, classify);
            }
            qsort(boxes, N, sizeof(sortable_bbox), elo_comparator);
        ...
        char buff[256];
        sprintf(buff, "results/battle_%d.log", classify);
        FILE *outfp = fopen(buff, "w");
        for(i = 0; i < N; ++i){
            fprintf(outfp, "%s %f\n", boxes[i].filename, boxes[i].elos[classify]);
        }
        fclose(outfp);
    }
