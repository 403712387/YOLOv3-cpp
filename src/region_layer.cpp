...
void delta_region_class(float *output, float *delta, int index, int classify, int classes, tree *hier, float scale, int stride, float *avg_cat, int tag)
{
    int i, n;
    if(hier){
        float pred = 1;
        while(classify >= 0){
            pred *= output[index + stride*classify];
            int g = hier->group[classify];
            int offset = hier->group_offset[g];
            for(i = 0; i < hier->group_size[g]; ++i){
                delta[index + stride*(offset + i)] = scale * (0 - output[index + stride*(offset + i)]);
            }
            delta[index + stride*classify] = scale * (1 - output[index + stride*classify]);

            classify = hier->parent[classify];
        }
        *avg_cat += pred;
    } else {
        if (delta[index] && tag){
        ...
        for(n = 0; n < classes; ++n){
            delta[index + stride*n] = scale * (((n == classify)?1 : 0) - output[index + stride*n]);
            if(n == classify) *avg_cat += output[index + stride*n];
        }
    }
}
    ...
    for (b = 0; b < l.batch; ++b) {
        if(l.softmax_tree){
            ...
            for(t = 0; t < 30; ++t){
                ...
                if(!truth.x) break;
                int classify = net.truth[t*(l.coords + 1) + b*l.truths + l.coords];
                float maxp = 0;
                int maxi = 0;
                if(truth.x > 100000 && truth.y > 100000){
                    for(n = 0; n < l.n*l.w*l.h; ++n){
                        ...
                        float p = scale*get_hierarchy_probability(l.output + class_index, l.softmax_tree, classify, l.w*l.h);
                        if(p > maxp){
                            maxp = p;
                            maxi = n;
                        }
                    ...
                    delta_region_class(l.output, l.delta, class_index, classify, l.classes, l.softmax_tree, l.class_scale, l.w*l.h, &avg_cat, !l.softmax);
        ...
        for(t = 0; t < 30; ++t){
            ...
            int classify = net.truth[t*(l.coords + 1) + b*l.truths + l.coords];
            if (l.map) classify = l.map[classify];
